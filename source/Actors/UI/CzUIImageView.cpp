...
CzXomlClassDef* CzUIImageView::ImageViewClassDef = NULL;

/**
 @fn	void CzUIImageView::InitClass()
...
void CzUIImageView::InitClass()
{
	// Create class properties
	ImageViewClassDef = new CzXomlClassDef();
	ImageViewClassDef->addProperty(new CzXomlClassProperty("MinZoom",			PT_Float,		CzUIImageView::_setMinZoom,				CzUIImageView::_getMinZoom));
	ImageViewClassDef->addProperty(new CzXomlClassProperty("MaxZoom",			PT_Float,		CzUIImageView::_setMaxZoom,				CzUIImageView::_getMaxZoom));
	ImageViewClassDef->addProperty(new CzXomlClassProperty("Zoom",				PT_Float,		CzUIImageView::_setZoom,				CzUIImageView::_getZoom));
	ImageViewClassDef->addProperty(new CzXomlClassProperty("ImageBrish",		PT_Resource,	CzUIImageView::_setImageBrush,			CzUIImageView::_getImageBrush));
	ImageViewClassDef->addProperty(new CzXomlClassProperty("Area",				PT_Vec2,		CzUIImageView::_setArea,				CzUIImageView::_getArea));

}
void CzUIImageView::ReleaseClass()
{
	if (ImageViewClassDef != NULL)
	{
		ImageViewClassDef->Release();
		delete ImageViewClassDef;
		ImageViewClassDef = NULL;
	};
}
...
bool CzUIImageView::_setMinZoom(IzXomlResource* target, const CzXomlProperty& prop, bool add)
{
	CzUIImageView* actor = (CzUIImageView*)target;
...
CzXomlProperty CzUIImageView::_getMinZoom(IzXomlResource* target)
{
	return CzXomlProperty(((CzUIImageView*)target)->getMinZoom());
}

bool CzUIImageView::_setMaxZoom(IzXomlResource* target, const CzXomlProperty& prop, bool add)
{
	CzUIImageView* actor = (CzUIImageView*)target;
...
CzXomlProperty CzUIImageView::_getMaxZoom(IzXomlResource* target)
{
	return CzXomlProperty(((CzUIImageView*)target)->getMaxZoom());
}

bool CzUIImageView::_setZoom(IzXomlResource* target, const CzXomlProperty& prop, bool add)
{
	CzUIImageView* actor = (CzUIImageView*)target;
...
CzXomlProperty CzUIImageView::_getZoom(IzXomlResource* target)
{
	return CzXomlProperty(((CzUIImageView*)target)->getZoom());
}

bool CzUIImageView::_setImageBrush(IzXomlResource* target, const CzXomlProperty& prop, bool add)
{
	CzUIImageView* actor = (CzUIImageView*)target;
	...
	if (prop.Type == PT_Resource)
		...
		brush = (CzBrushImage*)actor->getScene()->getResourceManager()->findResource(CzString::CalculateHash((const char*)prop.p_data), CzHashes::Brush_Hash);
	if (brush != NULL)
	{
		CzUIIcon* icon = actor->getIcon();
		if (icon != NULL)
			icon->SetFromBrush(brush);
	}
	else
	{
		CzDebug::Log(CZ_DEBUG_CHANNEL_WARNING, "ImageView - setProperty - Could not find named brush in the resources - ", (const char*)prop.p_data, actor->getDebugInfo().c_str());
		return false;
	}
...
CzXomlProperty CzUIImageView::_getImageBrush(IzXomlResource* target)
{
	CzUIImageView* actor = (CzUIImageView*)target;
	CzBitmapSprite* sprite = (CzBitmapSprite*)actor->getVisual();
	if (sprite == NULL)
		return CzXomlProperty();

	return CzXomlProperty(sprite->getBrush());
}

bool CzUIImageView::_setArea(IzXomlResource* target, const CzXomlProperty& prop, bool add)
{
	CzUIImageView* actor = (CzUIImageView*)target;
...
CzXomlProperty CzUIImageView::_getArea(IzXomlResource* target)
{
	return CzXomlProperty(((CzUIImageView*)target)->getArea());
}
...
CzUIImageView::CzUIImageView() : CzUIBase(), Icon(NULL), Zoom(1.0f), MinZoom(1.f), MaxZoom(1.0f), PrevZoom(0)
{
...
bool CzUIImageView::Update(float dt)
{
	if (!IsVisible)
		return CzUIBase::Update(dt);

	// Set clip rect for the view
	CzVec4 rc;
	...
	p2.x -= (Position.x);
	p2.y -= (Position.y);
	CzActor* linked = LinkedTo;
	while (linked != NULL)
	{
		CzVec2 p = linked->getPosition();
		p1.x -= p.x;
		p1.y -= p.y;
		p2.x -= p.x;
		p2.y -= p.y;
...
bool CzUIImageView::UpdateVisual()
{
...
void CzUIImageView::NotifyBeginTouch(int index, int x, int y, bool allow_bubble)
{
	if (!Enabled)
		return;

	if (!Icon->getVisual()->HitTest((float)x, (float)y))
	{
...
void CzUIImageView::NotifyEndTouch(int index, int x, int y, bool allow_bubble)
{
	if (!Enabled)
		return;
...
/**
 @fn	int CzUIImageView::LoadFromXoml(IzXomlResource* parent, bool load_children, CzXmlNode* node)
 ...
 nodes contained within the actor will also be loaded. Note that as CzUIImageView is derived from CzUIBase, CzUIBase::LoadFromXoml() will also be called to load CzUIBase specific properties. 
...
/**
 @fn	bool CzUIImageView::getProperty(const char* property_name, CzXomlProperty& prop)
 ...
 CzUIImageView is derived from CzUIBase, all CzUIBase properties will also be checked against.
...
bool CzUIImageView::getProperty(unsigned int property_name, CzXomlProperty& prop)
{
	if (CzUIBase::getProperty(property_name, prop))
		return true;

	int err = IzXomlResource::getProperty(ImageViewClassDef, this, property_name, prop);
	if (err == IzXomlResource::NotFound)
		return false;	// property not found

	if (err < IzXomlResource::NotFound)
	{
		CzXomlClassProperty* cprop = ImageViewClassDef->findProperty(property_name);
		CzDebug::Log(CZ_DEBUG_CHANNEL_WARNING, "ImageView - getProperty - Cannot get this property type, check the properties format - ", cprop->getName().c_str(), DebugInfo.c_str());
	}
...
/**
 @fn	bool CzUIImageView::setProperty(unsigned int property_name, const CzXomlProperty& data, bool delta)

 @brief	Sets the named property of the UIImageView.
    
 Sets the named property of this UIImageView. The properties value supplied will be converted. If delta is set to true then the existing value of the property will 
 ...
 @return	true if it succeeds, false if property does not exist.
 */
bool CzUIImageView::setProperty(unsigned int property_name, const CzXomlProperty& data, bool delta)
{
	if (CzUIBase::setProperty(property_name, data, delta))
		return true;

	int err = IzXomlResource::setProperty(ImageViewClassDef, this, property_name, data, delta);
	if (err == IzXomlResource::NotFound)
		return false;	// property not found
	...
	{
		CzXomlClassProperty* cprop = ImageViewClassDef->findProperty(property_name);
		CzDebug::Log(CZ_DEBUG_CHANNEL_WARNING, "ImageView - setProperty - Cannot set this property type, check the properties format - ", cprop->getName().c_str(), DebugInfo.c_str());
	}
...
/**
 @fn	bool CzUIImageView::setProperty(unsigned int property_name, const CzString& data, bool delta)
 ...
 will be added to instead of replaced. Note that as CzUIImageView is derived from CzUIBase, all CzUIBase properties will also be checked against.
...
bool CzUIImageView::setProperty(unsigned int property_name, const CzString& data, bool delta)
{
	if (CzUIBase::setProperty(property_name, data, delta))
		return true;
...
/**
 @fn	bool CzUIImageView::UpdateFromAnimation(CzAnimInstance *animation)
 ...
 animations frame data into its target property. Note that as CzUIImageView is derived from CzUIBase, all CzUIBase properties will also be checked against.
...
bool CzUIImageView::UpdateFromAnimation(CzAnimInstance *animation)
{
	if (CzUIBase::UpdateFromAnimation(animation))
		return true;

	int err = IzXomlResource::setProperty(ImageViewClassDef, this, animation);
	if (err == IzXomlResource::NotFound)
		return false;	// property not found

	if (err < IzXomlResource::NotFound)
	{
		CzXomlClassProperty* cprop = ImageViewClassDef->findProperty(animation->getTargetPropertyHash());
		CzDebug::Log(CZ_DEBUG_CHANNEL_WARNING, "ImageView - UpdateFromAnimation - Error setting the property type, check the properties format - ", cprop->getName().c_str(), DebugInfo.c_str());
	}
...
/**
 @fn	bool CzUIImageView::UpdateBinding(unsigned int property_name, CzXomlVariable* var)
...
bool CzUIImageView::UpdateBinding(unsigned int property_name, CzXomlVariable* var)
{
	...
	int err = IzXomlResource::setProperty(ImageViewClassDef, this, property_name, var);
	if (err == IzXomlResource::NotFound)
		return false;	// property not found
	...
	{
		CzXomlClassProperty* cprop = ImageViewClassDef->findProperty(property_name);
		CzDebug::Log(CZ_DEBUG_CHANNEL_WARNING, "ImageView - UpdateBindings - Error setting the property type, check the properties format - ", cprop->getName().c_str(), DebugInfo.c_str());
	}
